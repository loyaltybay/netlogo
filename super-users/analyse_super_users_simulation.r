...
d$message.spread.chance = str_c("p(spread) ", d$message.spread.chance)
...
g = ggplot(summ) + 
    ...
    facet_grid(super.user.node.degree ~ average.node.degree + message.spread.chance)+
    labs(title = "Time vs message spread %", x = 'time')
